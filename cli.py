...
class MMPExamCli(cmd.Cmd):
    def __init__(self, tag):
        ...
        self.archive_formats = [".zip", ".gz", ".tar.gz", ".tar", ".rar", ".7z"]
    ...
    def do_load_exam_files(self, arg):
        ...
        receive(self.service, self.date_query, prefix, attachments=True)
        self._unzip(prefix)
    ...
    def do_load_messages(self, arg):
        ...
        print("noks = {noks}, nfails = {nfails}".format(noks=noks, nfails=nfails))
    ...
    def _send_test(self, cc):
        ...
        for pid in persons:
            name = "{} {}".format(persons[pid]["last_name"], persons[pid]["first_name"])
    ...
    def _unzip(self, prefix):
        for root, dirs, files in os.walk(prefix):
            for filename in files:
                flag = any(el in filename for el in self.archive_formats)
                if flag:
    ...
    def _copy_studfiles(self, fname_begin, root, files):
        ...
        if self._filetype_case(files):
            ...
            src_filenames = [
                el for el in files if (".pdf" in el or ".jpg" in el or ".jpeg" in el)
            ]
            for index, src_filename in enumerate(src_filenames):
                src = os.path.join(root, src_filename)
                ext = os.path.splitext(src_filename)[1]
                dst = os.path.join(
                    self.path_imgs, "{} {}{}".format(fname_begin, index, ext)
                )
                copyfile(src, dst)
    ...
    def _send_options_to_person(self, poption, cc):
        ...
        body = self.options_body_template.format(name=poption["name"], option=option[0])
        send(
            self.service,
            poption["email"],
            self.options_subject,
            body,
            attachment=option[1],
            cc=cc,
        )
    ...
    def _set_results(self):
        ...
        for pid in persons:
            ...
            name = "{} {}".format(lname, persons[pid]["first_name"])
    ...
    def _send_results(self, path, cc):
        ...
        if ns_length > 0:
            print(data["not sent"])
            print(
                "There are some persons that will not receive their results (see list above)."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--tag", type=str, default="test", help="Mailing tag")
