...
def download_attachments(service, message, prefix, result):
    ...
    for part in message['payload']['parts']:
        ...
        if ('application' in part['mimeType'] or
            'image' in part['mimeType']):
...
def process_parts(result, mid, message):
    ...
    for part in message['payload']['parts']:
        if (part['partId'] == '0' and
            part['mimeType'] == 'text/plain'):
            result[mid]['message'] = base64.urlsafe_b64decode(part['body']['data']).decode('utf-8')
            
        if part['mimeType'] == 'multipart/alternative':
            for subpart in part['parts']:
                if subpart['mimeType'] == 'text/plain':        
                    result[mid]['message'] = base64.urlsafe_b64decode(subpart['body']['data']).decode('utf-8')
...
def receive(service, query, prefix=None, labels = ['INBOX', 'SPAM'],
           attachments = False):
